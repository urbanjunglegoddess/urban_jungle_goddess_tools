/**
 * WCAG 2.1 AA audit of the built Focus Window pages with axe-core.
 * Focus is deliberately single-theme — it is a dark room by design — so unlike
 * the other apps there is no light pass here.
 *
...
for (const [label, url, theme] of [
  ["index", "http://localhost:4322/", null],
  ["fit", "http://localhost:4322/fit", null],
  ["planner", "http://localhost:4322/planner", null],
  ["combined", "http://localhost:4322/combined", null],
  ["live", "http://localhost:4322/live", null],
]) {
  ...
  await page.waitForTimeout(700); // the island renders the compare grid and plan on load
...
await browser.close();
server.close();
console.log(total === 0 ? "\n✓ zero WCAG 2.1 AA violations across all five pages" : `\n✗ ${total} violation(s)`);
